...
- **Capture history** with **SQLite + FTS5** at `%LOCALAPPDATA%\Snapture\history\index.db`. Every capture auto-tagged with foreground process + window title, plus local named projects, dominant-color signatures, and perceptual hashes for project organization, color-similarity search, and near-duplicate filtering. The History window supports extended multi-select assignment without moving original files, `.snapture-library` backup/restore, and FTS5 search across OCR text, process name, and window title. Right-click → Open in editor / Pin / Run OCR / Reveal / Delete.
