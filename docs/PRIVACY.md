...
Snapture has no automatic update check in this release. There is no scheduled poll. There is no toast that asks you to update. The latest release is always at https://github.com/SysAdminDoc/Snapture/releases — check it when you want.

A future release may add an opt-in update check that resolves `https://api.github.com/repos/SysAdminDoc/Snapture/releases/latest`. If and when that ships:

- It will be off by default.
- It will only fire when you explicitly click an "Check for updates" button.
- It will store nothing locally about the request.
- The User-Agent will identify Snapture and the version, nothing else.
...
- Crash logs: `%APPDATA%\Snapture\crashlog.txt`.
